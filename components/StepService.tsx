        ...
        {activeTab === 'one-time' ? (
          <>
            {[
              ...
              { id: ServiceType.DEEP_CLEAN, label: "Deep Clean", desc: "Popular first time", badge: "Popular" },
              ...
              >
                {opt.badge && (
                  <span className="absolute -top-3 right-4 bg-amber-500 text-white text-[10px] font-bold px-2 py-1 rounded-full uppercase tracking-wider shadow-sm">
                    {opt.badge}
                  </span>
                )}
                <div className={`font-bold ${service.type === opt.id ? 'text-brand-700' : 'text-slate-800'}`}>
                  {opt.label}
                </div>
                <div className="text-xs text-slate-500 mt-1">{opt.desc}</div>
                {service.type === opt.id && (
                  <div className="absolute right-3 bottom-3 text-brand-600">
                    <CheckCircleIcon />
                  </div>
                )}
              </button>
          ...
          <>
            {[
              ...
              >
                {opt.badge && (
                  <span className="absolute -top-3 right-4 bg-green-500 text-white text-[10px] font-bold px-2 py-1 rounded-full uppercase tracking-wider shadow-sm">
                    {opt.badge}
                  </span>
                )}
                <div className="flex justify-between items-start">
                  <div className={`font-bold ${service.frequency === opt.id ? 'text-brand-700' : 'text-slate-800'}`}>
                    {opt.label}
                  </div>
                  <span className="bg-brand-100 text-brand-700 text-[10px] font-bold px-2 py-1 rounded">
                    {opt.discount}
                  </span>
                </div>
                <div className="text-xs text-slate-500 mt-1">{opt.desc}</div>
